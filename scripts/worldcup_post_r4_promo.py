"""
4강(준결승) 진출자 홍보 Reel — HP(HTML→Playwright→MP4).

8강 결과로 확정된 4강 진출자 4명과 준결승 대진을 대형 배틀 카드로 시각화.
투표 CTA 는 "오후 5시(17:00) 준결승 투표 시작" (아직 투표 오픈 전이므로 '진행중' 문구 금지).
실행: python scripts/worldcup_post_r4_promo.py [--dry-run]
"""
...
TOPIC_ID = "worldcup_r4_entrants_promo"
...
<body>
  <div class="header">
    <div class="pill">4강 진출 확정</div>
    <h1>준결승<br>대진</h1>
    <div class="sub">오늘 오후 5시 준결승 투표 시작</div>
  </div>
{cards}
  <div class="footer">
    <div class="cta-box">
      <div class="cta-main">오후 5시 투표 오픈</div>
      <div class="cta-sub">팔로우 + 알림 ON → 준결승 시작 즉시 알림</div>
    </div>
...
def main():
    ...
    matches = bracket.get("rounds", {}).get("R4", {}).get("matches", [])
    if not matches:
        print("❌ R4 매치 없음 (fix_bracket_r8_winners 먼저 실행 필요)")
        return 1

    out_dir = ROOT / "output_enter" / "publish" / "worldcup_r4_promo"
    out_dir.mkdir(parents=True, exist_ok=True)
    html_path = out_dir / "r4_promo.html"
    ...
    caption = (
        "걸그룹 월드컵 4강 진출 확정!\n\n"
        f"준결승 대진: {pairs}\n\n"
        "오늘 오후 5시 준결승 투표가 시작됩니다.\n"
        "팔로우 + 알림 ON → 준결승 시작 즉시 알림\n\n"
        "#걸그룹월드컵 #4강 #준결승 #케이팝 #kpop #아이돌투표 "
        + " ".join("#" + n for n in names)
    )
    media_id = publisher.post_reel(video_url, caption, cover_url=None, share_to_feed=True)
    print(f"✅ 4강 진출자 홍보 릴스 게시 완료! {media_id}")

    post_ledger.record_results([{
        "ok": True,
        "topic_id": TOPIC_ID,
        "title": "걸그룹 월드컵 4강 진출자 홍보",
        "style": "worldcup_r4_promo",
        "seed": None,
        "media_id": media_id,
        "platform": "instagram",
    }])
